...
export default function UnitSyllabus({ syllabus, unitName }: Props) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="border-t border-slate-100 p-5 space-y-6">
          ...
          {syllabus.rules && syllabus.rules.length > 0 && (
            <div>
              ...
              <ul className="list-disc space-y-1 pl-5 text-slate-600">
                {syllabus.rules.map((r, i) => <li key={i}>{r}</li>)}
              </ul>
            </div>
          )}
        </div>
              ...
              <div className="space-y-3 mt-3">
                {(day.activities || []).map((act, j) => (
                  <div key={j} className="rounded-lg border-l-4 border-blue-500 bg-blue-50 p-4">
                    ...
                    {act.overview && <p className="text-sm text-slate-700">{act.overview}</p>}
                    {act.rules && <ul className="mt-2 list-disc pl-5 text-sm text-slate-600">{act.rules.map((r, k) => <li key={k}>{r}</li>)}</ul>}
                  </div>
                ))}
              </div>
            </div>
      ...
      <details className="group rounded-xl border border-slate-200 bg-white shadow-sm">
        <summary className="flex cursor-pointer items-center justify-between bg-slate-50 p-4 font-bold text-slate-800 list-none rounded-xl group-open:rounded-b-none">
          <span className="flex items-center gap-2"><MessageCircle className="h-5 w-5 text-blue-600" /> Vocab & Discussion</span>
          <span className="text-blue-500 group-open:rotate-180 transition-transform">▼</span>
        </summary>
        <div className="border-t border-slate-100 p-5 space-y-6">
          {syllabus.key_terms && syllabus.key_terms.length > 0 && (
            <div>
              <h3 className="mb-3 text-lg font-bold text-slate-800">Vocabulary</h3>
              <ul className="space-y-2 text-slate-600 list-disc pl-5">
                {syllabus.key_terms.map((v, i) => <li key={i}><strong>{v.term}</strong>: {v.definition}</li>)}
              </ul>
            </div>
          )}
        </div>
